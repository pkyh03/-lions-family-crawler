...
# "우천순연", "미세먼지" 등)도 놓치지 않도록 취소 판정에 쓰는 키워드를 넓게 잡는다.
...
def crawl_full_schedule():
    ...
    for game_date, g in sorted(by_date.items()):
        ...
        # KBO 비고란 문구가 정확히 "취소"/"노게임"이 아닌 경우(예: "그라운드사정",
        # "우천순연", "미세먼지" 등)도 놓치지 않도록 넓힌 CANCEL_KEYWORDS로 판정한다.
...
def crawl_schedule_and_results():
    ...
    try:
        ...
        if not target:
            ...
            if not info:
                log("  - 오늘 삼성 경기 없음")
                # 과거 버그(오늘 날짜 오판 등)로 오늘자 행이 잘못 남아있을 수
                # 있으니, 실제로 오늘 경기가 없다고 확인되면 안전하게 정리한다.
                sb_delete("games", f"game_date=eq.{today.isoformat()}")
                return
            if not any(kw in info["remark"] for kw in CANCEL_KEYWORDS):
...
def ensure_player_registered(player_name, game_date):
    # _known_player_cache로 "이 실행에서 이미 이 날짜까지는 처리했는지"를
    # 추적한다. crawl_full_schedule()이 날짜 오름차순으로 경기를 처리하기
    # 때문에, 예전에는 이름만 보고 무조건 skip해서 최신 경기 날짜가
    # last_played_date에 반영되지 않고 스캔 구간 중 가장 오래된 날짜에
    # 고정되는 버그가 있었다 - 그 때문에 매일 뛰는 주전 선수들까지
    # "20일간 미출전"으로 오판되어 전원 비활성 처리되는 사고가 있었다.
    # DB 조회(existing) 자체는 매번 다시 하되, KBO 프로필 재조회 같은
    # 무거운 작업만 아래에서 자연히 건너뛴다 (이미 DB에 있으면 그쪽으로
    # 안 감).
    prev_date = _known_player_cache.get(player_name)
    if prev_date is not None and game_date <= prev_date:
        return
    _known_player_cache[player_name] = game_date
...
def sync_ticket_events():
    """삼성 라이온즈는 티켓을 홈 3연전(시리즈) 단위로 한꺼번에 열고, 오픈
    시점은 "시리즈 첫 경기일 기준 7일 전 오전 11시"로 거의 고정되어 있다
    (주말 3연전=전주 금요일, 평일 3연전=전주 화요일, 결과적으로 항상 7일 전).
    티켓링크 자체는 로그인 필요 + JS 렌더링 SPA라 스크래핑이 불안정하고
    약관 이슈 소지도 있어서, 대신 이미 동기화된 games 테이블의 홈경기
    일정에서 이 규칙으로 오픈일을 계산해 채운다. 매번 계산으로 다시 만드는
    데이터라, 통째로 비우고 새로 채운다.
    주의: 개막전이나 특별 이벤트 경기는 이 규칙에서 벗어나 더 일찍 열릴 수
    있다 - 그런 경우는 실제 오픈일보다 늦게(부정확하게) 표시될 수 있음.
    """
    log("티켓 예매 일정 계산 시작")
    try:
        today = datetime.date.today()
        home_games = sb_select(
            "games",
            "select=game_date,opponent,status&is_home=eq.true&order=game_date.asc",
        )
        home_games = [g for g in home_games if g["status"] != "cancelled"]

        # 연속된 날짜 + 같은 상대팀을 하나의 "시리즈"로 묶는다.
        series_list = []
        cur = None
        for g in home_games:
            d = datetime.date.fromisoformat(g["game_date"])
            if cur and cur["opponent"] == g["opponent"] and (d - cur["last_date"]).days == 1:
                cur["last_date"] = d
                cur["count"] += 1
            else:
                if cur:
                    series_list.append(cur)
                cur = {"opponent": g["opponent"], "first_date": d, "last_date": d, "count": 1}
        if cur:
            series_list.append(cur)
        ...
        for s in series_list:
            open_date = s["first_date"] - datetime.timedelta(days=7)
            if s["last_date"] < today:
                ...
                status = "upcoming"
            wd = WEEKDAY_KR[open_date.weekday()]
            title = f"{s['opponent']} {s['count']}연전 티켓 오픈"
            event_date = (
                f"{open_date.month}.{open_date.day}({wd}) 11:00 · "
                f"{s['first_date'].month}.{s['first_date'].day}~{s['last_date'].month}.{s['last_date'].day} 홈경기"
            )
            rows_out.append({
                "title": title, "event_date": event_date, "status": status,
                "sort_order": (s["first_date"] - today).days,
            })

        sb_delete("ticket_events", "id=gt.0")
        sb_upsert("ticket_events", rows_out, on_conflict="id")
        log(f"  - 티켓 예매 일정 계산 완료 ({len(rows_out)}건)")
    except Exception as e:
        log(f"  ! 티켓 예매 일정 계산 실패: {e}")
